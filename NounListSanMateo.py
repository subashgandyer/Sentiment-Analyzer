...
def main():
	...
	data = pd.read_csv('SanMateoReviewsOnly.csv',sep='|')
	#print data.head()
	revs = data.loc[:,['r_name','reviewtext']]
	#print revs.head()
	#print revs.count()
	...
	revs['text'] = revs['reviewtext'].apply(lambda x: x.decode('utf-8').encode('ascii', 'ignore').strip())
	
	revs['r_name'] = revs['r_name'].apply(lambda x: x.decode('utf-8').encode('ascii', 'ignore').strip())
 ...
 #                                         	n = len(idfs))
	...
	conn = sqlite3.connect('nouns_SanMateo.db')
	...
	conn.execute("DROP TABLE IF EXISTS NOUNS")
	conn.execute('''CREATE TABLE NOUNS
	       (ID INT PRIMARY KEY     	NOT NULL,
	       NOUNS       TEXT   	NOT NULL)''')
	print "Table created successfully"
		...
		for sent in sentences:
			...
			if len(dish2) != 0:
				for t in dish2:
					...
					if noun not in dlist:
						nouns.append(noun)
						nouns_list.append(noun)
			print 'Nouns after 3rd iteration ', nouns
			dlist = '\t'.join(nouns)
			...
			if len(dish1) != 0:
				for t in dish1:
					if t not in dlist:
						nouns.append(t)
						print nouns, type(nouns), nouns[0], type(nouns[0])
						#conn.execute("INSERT INTO NOUNS VALUES (?, ?)", (cnt, nouns[0]))
						nouns_list.append(t)
						#cnt += 1

			print 'Nouns after last iteration ', nouns
			for i in range(len(nouns)):
				conn.execute("INSERT INTO NOUNS VALUES (?, ?)", (cnt, nouns[i]))
				cnt += 1
			conn.commit()	
			#print 'Nouns for the sentence ', nouns_list
